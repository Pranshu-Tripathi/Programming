/*
Algorithm is used to find the shortest path from one node to all other nodes in a weighted graph
*/
/*
...
void printVar(ll val, string name)
{
  cout << name << " : " << val << endl;
}
...
void _run()
{
    ...
    while(!pq.empty())
    {
        ...
        {
            if(curr_d + relation.second < dist[relation.first])
            {
                dist[relation.first] = curr_d + relation.second;
                pq.push({curr_d+relation.second,relation.first});
            }
        }
    }
    display(dist);
