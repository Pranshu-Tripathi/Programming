...
int getInorderIndex(int val,int inorder[])
{
    for(int i = 0 ; i < sizeof(inorder) ; i++)
    {
        if(val == inorder[i])
            return i;
    }
...
Node* binaryTreeFromPreorderInorder(int preorder[], int inorder[], int start, int end)
{
    if(start > end)
        return nullptr;
    
    Node* node = createNewNode(preorder[preorderIndex++]);
    if(start == end)
        return node;
    
    int inorderIndex = getInorderIndex(node->val,inorder);

    node->left = binaryTreeFromPreorderInorder(preorder,inorder,start,inorderIndex-1);
    node->right = binaryTreeFromPreorderInorder(preorder,inorder,inorderIndex+1,end);

    return node;
}

int getHeightIteratively(Node * root)
{
    if(root == nullptr)
        return NULL;
