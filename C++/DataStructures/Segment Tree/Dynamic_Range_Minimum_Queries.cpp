...
struct segmentTree{
    ...
    void assemble(vector<ll> &v, int l,int r,int node)
    {
        if(l > r)
            return;
        if(l == r)
    ...
    void update(int idx, ll val, int l, int r, int node)
    {
        if(l > r)   
            return;
        if(l == r)
        ...
        int mid = (l+r)/2;
        if(idx <= mid)
            update(idx,val,l,mid,2*node+1);
        else
            update(idx,val,mid+1,r,2*node+2);
        tree[node] = min(tree[2*node+1],tree[2*node+2]);
    }

    ll rangeQuery(int node, int l, int r, int ql ,int qr)
    {
        if(l > r || ql > qr || l > qr || r < ql )   // invalid intervals
            return INF;
        if(l >= ql && r <= qr)  // complete overlap
            return tree[node];
        int mid = (l+r)/2;
        return min(rangeQuery(2*node+1,l,mid,ql,qr),rangeQuery(2*node+2,mid+1,r,ql,qr));
    }

    void changeTree(int idx, ll val)
    {
        idx--;
